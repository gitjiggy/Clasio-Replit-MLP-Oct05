...
export class DatabaseStorage implements IStorage {
  ...
  private async initializeDefaults() {
    try {
      ...
      // TODO: Run folder repair to fix any redundant folder names
      // await this.repairRedundantFolders();
    } catch (error) {
...
🔧 GCS Path Reconciliation ${dryRun ? '(DRY RUN)' : '(COMPLETED)'}
  ...
  // Method to repair redundant folder names and merge duplicates for a specific user
  async repairRedundantFoldersForUser(userId: string): Promise<{ repaired: number; merged: number; errors: string[] }> {
    try {
      
      console.log("🔧 Starting repair of redundant folder names...");
      
      // Define redundant folder patterns that need repair
      const redundantPatterns = [
        'Business Report', 'Tax Document', 'Technical Docs', 'Immigration Document',
        'Medical Record', 'Employment Document', 'Legal Document', 'Other'
      ];
      ...
      for (const pattern of redundantPatterns) {
        try {
          // Find all folders with this redundant name
          const redundantFolders = await db
            .select()
            .from(folders)
            .where(
              and(
                eq(folders.name, pattern),
                eq(folders.isAutoCreated, true),
                eq(folders.userId, userId),
          ...
          if (redundantFolders.length > 0) {
            console.log(`🔍 Found ${redundantFolders.length} redundant "${pattern}" folders`);
            
            for (const folder of redundantFolders) {
              // Get documents in this folder to generate a better name
              const docsInFolder = await db
                ...
                .limit(3); // Sample first 3 documents
              
              if (docsInFolder.length > 0) {
                // Generate a better folder name based on documents
                const betterName = this.generateBetterFolderName(pattern, docsInFolder);
                
                if (betterName !== pattern) {
                  // Check if a folder with the better name already exists in the same parent
                  const existingBetterFolder = await db
                    .select()
                    .from(folders)
                    .where(
                      and(
                        eq(folders.name, betterName),
                  ...
                  if (existingBetterFolder.length > 0) {
                    // Merge documents into existing better folder
                    await db
                      .update(documents)
                      .set({ folderId: existingBetterFolder[0].id })
                      .where(eq(documents.folderId, folder.id));
                    
                    // Delete the redundant folder
                    await db.delete(folders).where(eq(folders.id, folder.id));
                    
                    console.log(`🔀 Merged "${pattern}" → "${betterName}"`);
                    merged++;
                  } else {
                    // Rename the folder to the better name
                    await db
                      .update(folders)
                      .set({ 
                        name: betterName,
                        documentType: betterName
                      })
                      .where(eq(folders.id, folder.id));
                    
                    console.log(`✏️ Renamed "${pattern}" → "${betterName}"`);
                    repaired++;
                  }
                }
              }
          ...
          const errorMsg = `Error repairing "${pattern}": ${error instanceof Error ? error.message : 'Unknown error'}`;
          errors.push(errorMsg);
          console.error(errorMsg);
        }
      }
      
      console.log(`🔧 Repair complete: ${repaired} renamed, ${merged} merged, ${errors.length} errors`);
      return { repaired, merged, errors };
      
    } catch (error) {
      console.error('Failed to repair redundant folders:', error);
      return { repaired: 0, merged: 0, errors: [error instanceof Error ? error.message : 'Unknown error'] };
    }
  }
  
  private generateBetterFolderName(originalName: string, sampleDocs: any[]): string {
    // Analyze document names to generate a more descriptive folder name
    const docNames = sampleDocs.map(doc => doc.name.toLowerCase());
    
    // Extract common themes and keywords
    if (originalName === 'Business Report') {
      if (docNames.some(name => name.includes('cyber') || name.includes('security'))) {
        return 'Cybersecurity Analysis';
      }
      if (docNames.some(name => name.includes('product') || name.includes('portfolio'))) {
        return 'Product Strategy';
      }
      if (docNames.some(name => name.includes('interview') || name.includes('mapping'))) {
        return 'User Research';
      }
      if (docNames.some(name => name.includes('launch') || name.includes('strategy'))) {
        return 'Launch Strategy';
      }
    }
    
    if (originalName === 'Tax Document') {
      const currentYear = new Date().getFullYear();
      if (docNames.some(name => name.includes('1099'))) {
        return `${currentYear} Tax Forms`;
      }
      return `${currentYear} Tax Documents`;
    }
    
    if (originalName === 'Technical Docs') {
      if (docNames.some(name => name.includes('test') || name.includes('notes'))) {
        return 'Development Notes';
      }
      return 'Technical Documentation';
    }
    
    if (originalName === 'Immigration Document') {
      if (docNames.some(name => name.includes('passport') || name.includes('id'))) {
        return 'Identity Documents';
      }
      if (docNames.some(name => name.includes('reg') || name.includes('registration'))) {
        return 'Registration Documents';
      }
    }
    
    // Default improvements for other types
    return originalName.replace(/Document$/, 'Documents');
  }
