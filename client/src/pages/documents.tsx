      ...
      <main className="flex-1 md:overflow-hidden flex flex-col overflow-hidden">
              ...
              <div className="relative flex-1 max-w-3xl">
                ...
                >
                  <img 
                    src={desktopMicIcon} 
                    alt="Voice Search" 
                    className={`h-32 w-32 transition-all object-contain ${
                      isVoiceListening 
        ...
        {(() => {
          ...
          {searchMode === "ai" && aiSearchResults && (
            <div className="mb-6">
              <div className="rounded-lg border p-4 mb-4 dark:bg-gray-900 dark:border-purple-500" style={{ backgroundColor: '#E2D8FF', borderColor: '#D0BFFF' }}>
                <div className="flex items-center gap-2 mb-2">
                  <Brain className="h-5 w-5 text-purple-500" />
                  <h3 className="text-sm font-semibold dark:text-purple-100" style={{ color: '#1E1E1E' }}>AI Search Results</h3>
                  <Badge variant="secondary" className="text-xs">
                    {aiSearchResults.totalResults} found
                  </Badge>
                </div>
                <div className="text-sm mb-2 dark:text-purple-200" style={{ color: '#1E1E1E' }}>
                      ...
                      {aiSearchResults.response.split('•').filter(part => part.trim()).map((part, index) => (
                        <div key={index} className={index === 0 ? 'mb-2' : 'flex items-start gap-2'}>
                          {index === 0 ? (
                            ...
                            <>
                              <span className="font-bold mt-0.5 min-w-[1.25rem] dark:text-purple-400" style={{ color: '#1E1E1E' }}>{index}.</span>
                ...
                {aiSearchResults.keywords && aiSearchResults.keywords.length > 0 && (
                  <div className="flex gap-1 flex-wrap">
                    <span className="text-xs dark:text-purple-300" style={{ color: '#1E1E1E' }}>Keywords:</span>
                ...
                >
                  <CardContent className="px-4 pt-4 pb-0 flex flex-col h-full overflow-hidden">
                    ...
                    {(document.aiSummary || document.overrideDocumentType || document.overrideCategory) && (
                      <div className="mb-0 p-2.5 rounded-lg border flex-shrink-0 max-h-[200px] overflow-y-auto scrollbar-thin scrollbar-track-transparent" style={{ backgroundColor: '#E2D8FF', borderColor: '#D0BFFF' }}>
                        <div className="flex items-center gap-1.5 mb-1.5 sticky top-0 pb-1" style={{ backgroundColor: '#E2D8FF' }}>
                          <Sparkles className="h-3 w-3 text-purple-500 dark:text-purple-400" />
                          <span className="text-xs font-light tracking-wide" style={{ color: '#1E1E1E' }}>
                            {document.aiSummary ? 'AI Analysis' : 'Classification'}
                          </span>
                        </div>
                        {document.aiSummary && (
                          <p className="text-xs mb-2 leading-relaxed font-light tracking-wide" style={{ color: '#1E1E1E' }}>{document.aiSummary}</p>
                        )}
                        {(document.aiDocumentType || document.overrideDocumentType || document.overrideCategory) && (
                          <div className="text-xs mb-2 space-y-1" style={{ color: '#1E1E1E' }}>
                            <div className="flex flex-col gap-0.5">
                              <div className="flex items-center justify-between">
                                ...
                                {document.overrideCategory ? (
                                  ...
                                  <span className="text-xs px-1 py-0.5 rounded font-medium whitespace-nowrap ml-2" style={{ backgroundColor: '#D0BFFF', color: '#1E1E1E' }} data-testid={`confidence-category-${document.id}`}>
                                    {formatConfidence(document.aiCategoryConfidence)}
                                  </span>
                                )}
                              </div>
                              ...
                              <div className="flex flex-col gap-0.5">
                                <div className="flex items-center justify-between">
                                  ...
                                  {document.overrideDocumentType ? (
                                    ...
                                    <span className="text-xs px-1 py-0.5 rounded font-medium whitespace-nowrap ml-2" style={{ backgroundColor: '#D0BFFF', color: '#1E1E1E' }} data-testid={`confidence-type-${document.id}`}>
                                      {formatConfidence(document.aiDocumentTypeConfidence)}
                                    </span>
                                  )}
                                </div>
                        ...
                        {document.aiKeyTopics && document.aiKeyTopics.length > 0 && (
                          <div className="flex flex-wrap gap-1 pt-1 border-t" style={{ borderColor: '#D0BFFF' }}>
                            {document.aiKeyTopics.map((topic, index) => (
                              <Badge key={index} variant="secondary" className="text-xs border-0" style={{ backgroundColor: '#D0BFFF', color: '#1E1E1E' }}>
                                {topic}
                              </Badge>
                            ))}
                          </div>
